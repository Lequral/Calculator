...
export const MathEvaluator = (function () {
	...
	function symbol(id, bp) {
		...
		s.prototype.val = function () {
			throw new Error(
				"Unknown operation with %1.".replace("%1", this.id)
			);
		};
	...
	bindEval("^", function () {
		return Math.pow(this.first.val(), this.second.val());
	});
	bindEval("<<", function () {
		return this.first.val() << this.second.val();
	});
	...
	bindEval("(", function () {
		...
		if (!(fn instanceof Function)) {
			throw new Error(
				"Unknown function %1.".replace("%1", this.first.value)
			);
		}
